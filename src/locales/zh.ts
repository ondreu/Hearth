...
export const zh: Translations = {
	...
	editors: {
		...
		tiles: {
			sizing: "按钮尺寸方式",
			sizingDesc:
				"按钮是随卡片一起缩放（就像卡片随面板缩放那样），还是无论卡片多大都保持固定的像素尺寸。" +
				"在此设置出现之前创建的卡片会继续使用固定方式，直到你手动切换；两种方式各自保留自己的尺寸，" +
				"因此切换回去后原来的样子会完好如初。",
			sizingScale: "随卡片缩放",
			sizingFixed: "固定尺寸（旧版）",
			across: "每行按钮数",
			acrossDesc:
				"卡片横向能放几个按钮，也就决定了单个按钮有多大：它始终是卡片的一个固定比例，无论卡片最终多大。" +
				"在排列模式下拖动右下角，仍可把一个按钮做成两三格宽（或高）——它始终对齐网格。",
		},
		links: {
			heading: "链接",
